...
try { const { data: screenerHtml } = await axios.get(screenerUrl); const $ = cheerio.load(screenerHtml);
...
} catch (err) { console.warn(⚠️ Screener fetch failed for ${symbol}, trying fallback...); }

try { const { data: fallbackHtml } = await axios.get(intradayUrl); const $$ = cheerio.load(fallbackHtml);
...
} catch (err) { console.warn(⚠️ Intradayscreener fallback also failed for ${symbol}); }

if (!stats.companyName) { return res.status(500).json({ error: 'Stock not found or error occurred.' }); }
